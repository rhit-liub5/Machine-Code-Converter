...
def pseudoinstruction_pass(asm_lines, pseudos_dictionary):
    ...
    for i, line in enumerate(asm_lines):
        ...
        label = ""
        if has_label(line):
            label, rest = split_out_label(line)
            label = label + ":"
            raw = rest
            if not raw: 
                ...
                result.append(label)
                continue
        split_inst = raw.strip().replace(",", " ").split()
        cmd = split_inst[0]
        if cmd in pseudos_dictionary.keys():
            new = pseudos_dictionary[cmd](raw, i)
            new[0] = f"{label} {new[0]}" 
            result.extend(new)
        else:
            result.append(line)
...
def Assemble(inst, line_num=0, labels=None):
    ...
    #DONE: Practical 1 - Implement this
    ...
    if cmd not in inst_to_types:
        raise BadInstruction(f"Unknown instruction: {cmd}")

    itype = inst_to_types[cmd]
    if itype == Types.R:
        return Assemble_R_Type(cmd, operands, line_num)
    
    if itype == Types.I:
        return Assemble_I_Type(cmd, operands, line_num)

    if itype == Types.S:
        return Assemble_S_Type(cmd, operands, line_num)
    
    if itype == Types.SB:
        return Assemble_SB_Type(cmd, operands, line_num,labels)
    
    if itype == Types.U:
        return Assemble_U_Type(cmd, operands, line_num)
    
    if itype == Types.UJ:
        return Assemble_UJ_Type(cmd, operands, line_num,labels)
...
def Assemble_R_Type(cmd, operands, line_num):
    ...
    # ensure instruction is actually R-type
    if cmd not in inst_to_types or inst_to_types[cmd] != Types.R:
        raise BadInstruction (" Instruction is not a valid R Type on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))

    # ensure number of arguments are correct for R type instruction
    if len(operands) != 3:
        raise BadOperands (" Incorrect number of args found in R Type on line" + 
                           " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # ensure base-offset notation is not used
    for op in operands:
        if "(" in op:
            raise BadOperands (" Base-offset notation found in R Type on line" +
                               " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))

    # get the opcode, func3, func7 field data from dict
    field_data = inst_to_fields[cmd]

    # get machine code for the operand registers
    rd = get_register_bin(operands[0])
    rs1 = get_register_bin(operands[1])
    rs2 = get_register_bin(operands[2])

    # combine all fields into a list to combine into machine code with helper
    inst_field_list = [field_data.func7, rs2, rs1, field_data.func3, rd, field_data.opcode]
...
def Assemble_I_Type(cmd, operands, line_num):
    ...
    # ensure instruction is actually I-type
    if cmd not in inst_to_types or inst_to_types[cmd] != Types.I:
        raise BadInstruction (" Instruction is not a valid I Type on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # call helpers/stay in main depending on args and type of instruction
    if cmd == "lw" or cmd == "jalr":
        # base offset format in I Type instr
        return Assemble_I_Type_base_offset(cmd, operands, line_num)
    elif len(operands) == 3:
        # make sure not base offset format
        for op in operands:
            if "(" in op:
                raise BadOperands (" Base-offset notation found in invalid I Type on line" +
                                " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
        
        # determine whether it's a shift argument or other basic I Type
        if is_shift_immediate_inst(cmd):
            return Assemble_I_Type_shift(cmd, operands, line_num)
        else:
            # non-shift, non-lw I-Type instruction
            # get the opcode, func3 field data from dict
            field_data = inst_to_fields[cmd]

            # get machine code for the operand registers
            rd = get_register_bin(operands[0])
            rs1 = get_register_bin(operands[1])
            imm = dec_to_bin(operands[2])

            # return machine code instruction
            return Assemble_I_Type_from_fields(imm, rs1, field_data.func3, rd, field_data.opcode, line_num)
    else:
        raise BadOperands(" Incorrect number of args found in I Type on line" +
                          " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
...
def Assemble_I_Type_shift(cmd, operands, line_num):
    ...
    # ensure I-type shift instruction
    if not is_shift_immediate_inst(cmd):
        raise BadInstruction (" Instruction is not a valid I Type Shift Immediate on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # ensure number of operands is correct
    if len(operands) != 3:
        raise BadOperands(" Incorrect number of args found in I Type Shift on line" +
                          " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # get func3 and opcode for I Type shift commands
    field_data = inst_to_fields[cmd]
    ...
    # ensure immediate validity
    immInt = int(operands[2])
    if immInt < 0 or immInt > 31:
        raise BadImmediate (" Immediate does not fall within Shift Immediate Type bounds " +
                            " on line %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    imm = dec_to_bin(operands[2])
    # set left seven bits of immediate to func7 constants on green sheet
    if cmd == "srai":
        finalImm = "0100000" + imm[7:]
    else:
        finalImm = "0000000" + imm[7:]

    # return binary instruction
    return Assemble_I_Type_from_fields(finalImm, rs1, field_data.func3, rd, field_data.opcode, line_num)
...
def Assemble_I_Type_base_offset(cmd, operands, line_num):
 ...
 """
    ...
    #DONE: Practical 1 - implement this
    # ensure it is an I-type base-offset instruction
    if cmd != "lw" and cmd != "jalr":
        raise BadInstruction (" Invalid I-Type base-offset instruction on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # ensure correct number of operands and parse instruction based on format used
    field_data = inst_to_fields[cmd]
    if len(operands) == 2:
        # base-offset format with no space
        rd = get_register_bin(operands[0])
        (imm, rs1) = parse_base_offset(operands[1])

        return Assemble_I_Type_from_fields(imm, rs1, field_data.func3, rd, field_data.opcode, line_num)
    elif len(operands) == 3:
        # normal 3-argument format OR base-offset with a space
        if "(" in operands[2]:
            # base-offset with a space yields rd, imm, (rs1)
            rd = get_register_bin(operands[0])
            imm = dec_to_bin(operands[1])
            l = len(operands[2])
            rs1 = get_register_bin(operands[2][1:l-1])
        else:
            # normal 3-argument format rd, rs1, imm
            rd = get_register_bin(operands[0])
            rs1 = get_register_bin(operands[1])
            imm = dec_to_bin(operands[2])
        ...
        raise BadOperands (" Invalid number of arguments in I-Type base-offset instruction" + 
                           " on line %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
...
def Assemble_I_Type_from_fields(imm, rs1, func3, rd, opcode, line_num):
    ...
    fields = [imm, rs1, func3, rd, opcode]
    return join_inst_fields_bin(fields)
...
def Assemble_S_Type(cmd, operands, line_num):
    ...
    # ensure instruction is actually S-type
    if cmd not in inst_to_types or inst_to_types[cmd] != Types.S:
        raise BadInstruction (" Instruction is not a valid S Type on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    ...
    if len(operands) == 2:
        ...
        (imm, rs1) = parse_base_offset(operands[1])
        fields = [imm[:7], rs2, rs1, field_data.func3, imm[7:], field_data.opcode]
        return join_inst_fields_bin(fields)
    elif len(operands) == 3:
        # normal 3-argument format OR base-offset with a space
        if "(" in operands[2]:
            # base-offset with a space yields rs2, imm, (rs1)
            rs2 = get_register_bin(operands[0])
            imm = dec_to_bin(operands[1])
            l = len(operands[2])
            rs1 = get_register_bin(operands[2][1:l-1])
        else:
            # normal 3-argument format rs2, rs1, imm
            rs2 = get_register_bin(operands[0])
            rs1 = get_register_bin(operands[1])
            imm = dec_to_bin(operands[2])
        ...
        raise BadOperands (" Invalid number of arguments in S-Type base-offset instruction" + 
                           " on line %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
...
def Assemble_SB_Type(cmd, operands, line_num, labels=None):
    ...
    #DONE: Practical 2 - implement this
    # ensure instruction is actually SB-type
    if cmd not in inst_to_types or inst_to_types[cmd] != Types.SB:
        raise BadInstruction (" Instruction is not a valid SB Type on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # ensure correct number of operands is provided
    if len(operands) != 3:
        raise BadOperands (" Invalid number of arguments in SB-Type instruction on line" + 
                           " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # get opcode, destination register address, and immediate binary fields
    field_data = inst_to_fields[cmd]
    rs1 = get_register_bin(operands[0])
    rs2 = get_register_bin(operands[1])
    # immediate or label is passed in as third argument
    imm = ""
    if is_int(operands[2]):
        imm += dec_to_bin(int(operands[2]) // 2)
    else:
        # third arg was a label, so calculate byte offset
        # first check that label is defined otherwise it is taken as a bad immediate according to tests
        if operands[2] not in labels.keys():
            raise BadImmediate (" Invalid immediate used in SB-Type instruction on line" + 
                            " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
        offset = label_to_offset(labels, operands[2], line_num)
        imm += dec_to_bin(int(offset) // 2)

    # join the fields together into a list and return machine code instruction
    fields = [imm[0], imm[2:8], rs2, rs1, field_data.func3, imm[8:], imm[1], field_data.opcode]
    return join_inst_fields_bin(fields)
...
def Assemble_U_Type(cmd, operands, line_num):
    ...
    #DONE: Practical 2 - implement this
    # ensure instruction is actually U-type
    if cmd not in inst_to_types or inst_to_types[cmd] != Types.U:
        raise BadInstruction (" Instruction is not a valid U Type on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # ensure correct number of operands is provided
    if len(operands) != 2:
        raise BadOperands (" Invalid number of arguments in U-Type instruction on line" + 
                           " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    ...
    # stitch together fields and return binary instruction
    fields = [imm, rd, field_data.opcode]
    return join_inst_fields_bin(fields)
...
def Assemble_UJ_Type(cmd, operands, line_num, labels):
    ...
    #DONE: Practical 2 - implement this
    # ensure instruction is actually UJ-type
    if cmd not in inst_to_types or inst_to_types[cmd] != Types.UJ:
        raise BadInstruction (" Instruction is not a valid UJ Type on line" +
                              " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # ensure correct number of operands is provided
    if len(operands) != 2:
        raise BadOperands (" Invalid number of arguments in UJ-Type instruction on line" + 
                           " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
    
    # get opcode, destination register address, and immediate binary fields
    field_data = inst_to_fields[cmd]
    rd = get_register_bin(operands[0])
    # immediate or label is passed in as second argument
    imm = ""
    if is_int(operands[1]):
        imm += dec_to_bin(int(operands[1]) // 2, 20)
    else:
        # second arg was a label, so calculate byte offset
        # first check that label is defined otherwise it is taken as a bad immediate according to tests
        if operands[1] not in labels.keys():
            raise BadImmediate (" Invalid immediate used in UJ-Type instruction on line" + 
                            " %s with args:\n\t%s%s\n" % (line_num, cmd, operands))
        offset = label_to_offset(labels, operands[1], line_num)
        imm += dec_to_bin(int(offset) // 2, 20)

    # join the fields together into a list and return machine code instruction
    fields = [imm[0], imm[10:20], imm[9], imm[1:9], rd, field_data.opcode]
    return join_inst_fields_bin(fields)
...
def parse_labels(asm_list):
    ...
    #DONE: Practical 2 - implement this
    # iterate through the lines, mapping labels to addresses and producing clean code
    clean_code = []
    label_to_address = {}
    for line in asm_list:
        line = line.strip()
        if ":" in line:
            # line is either a label or label and instruction
            label = ""
            index = 0
            ...
            if line.endswith(":"):
                # just a label
                label = line[:-1]
                # map label to index/address of the next line
                index = len(clean_code)
            else:
                # label and instruction - split parts
                label, instr = line.split(":")
                # map label to code line and add clean code
                index = len(clean_code)
                clean_code.append(instr.strip())
            
            # check that label is not a duplicate, and if not then add to dict
            if label in label_to_address.keys():
                raise BadLabel (" Duplicate label name found in instruction on line" + 
                                " %s with code:\n\t%s\n" % (index, line))
            else:
                # add to dict
                label = label.strip()
                label_to_address[label] = index_to_address(index)

        else:
            # line is a pure instruction
            clean_code.append(line)
    
    # Reminder: you this should return 2 things, but probably not these two
    return (tuple(clean_code), label_to_address)
...
def label_to_offset(labels, label, instruction_index):
    ...
    #DONE: Practical 2 - implement this
    current_pc = index_to_address(instruction_index)
    if label not in labels.keys():
        # label is undefined
        raise BadLabel (" Invalid label name found in instruction on line %s" % (instruction_index))
    target_pc = labels[label]
    return target_pc - current_pc
